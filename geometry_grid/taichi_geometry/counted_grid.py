...
class CountedGrid:
    def __init__(
        ...
        self.total_cells, self.total_entries = [int(n) for n in self._count_objects()]
    ...
    def from_torch(grid: Grid, torch_objects: TensorClass, grid_chunk=8):
        ...
        return CountedGrid(
            grid, objects, object_types, grid_chunk, device=torch_objects.device
        )
    ...
    def _compute_prefixes(self):
        ...
        for i in ti.grouped(self.chunks):
            ...
            ti.loop_config(serialize=True)
            for j in ti.grouped(
                ti.ndrange(self.grid_chunk, self.grid_chunk, self.grid_chunk)
            ):
    ...
    @ti.kernel
    def _active_cells(
        self, cells: ti.template(), counts: ti.types.ndarray(ti.i32, ndim=1)
    ):
    ...
    def active_cells(self) -> Tuple[torch.Tensor, torch.Tensor]:
        cells = ivec3.field()
        l = ti.root.dynamic(ti.i, self.total_cells, chunk_size=self.total_cells).place(
            cells
        )
