...
def _check_shape(data_name, struct_shape, ti_struct: ti.lang.struct.StructType):
    for k, v in ti_struct.members.items():
        if not k in struct_shape:
            raise TypeError(
                f"Missing field in struct {k} in {data_name} {sorted(ti_struct.members.keys())} vs. {sorted(struct_shape.keys())}"
            )
        ...
        if isinstance(v, ti.lang.matrix.VectorType):
            ...
            if shape != v.get_shape():
                raise TypeError(
                    f"Expected {k} to have shape {v.get_shape()}, got {shape}"
                )

            if dtype != taichi_torch[v.dtype]:
                raise TypeError(
                    f"Expected {k} to have dtype {taichi_torch[v.dtype]}, got {dtype}"
                )
...
def taichi_shape(ti_type):
    ...
    if isinstance(ti_type, ti.lang.matrix.VectorType) or isinstance(
        ti_type, ti.lang.matrix.MatrixType
    ):
...
def tensorclass_field(data: TensorClass, dtype: ti.lang.struct.StructType):
    if data.shape_info != taichi_shape(dtype):
        raise TypeError(
            f"Expected shapes don't match for {data.__class__.__name__}:\n{data.shape_info} and \n{taichi_shape(dtype)}"
        )
...
def converts_to(data: TensorClass):
    assert (
        data.__class__ in conversions
    ), f"Unsupported type {data.__class__}, options are {list_conversions()} use register_conversion"
