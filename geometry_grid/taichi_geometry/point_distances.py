...
@ti.func
def _min_point_objects(
    point: ti.math.vec3, obj_struct: ti.template(), obj_arr: ti.template()
):
...
def min_distances_kernel(obj_struct):
    ...
    @ti.kernel
    def k(
        obj_arr: ti.types.ndarray(dtype=ti.types.vector(size, ti.f32), ndim=1),
        points: ndarray(ti.math.vec3),
        radius: ti.f32,
        distances: ndarray(ti.f32),
        indices: ndarray(ti.i32),
    ):
        for j in range(points.shape[0]):
            indices[j], distances[j] = _min_point_objects(
                points[j], radius, obj_struct, obj_arr
            )
...
def min_point_distances(objects: TensorClass, points: torch.Tensor):
    distances = torch.full(
        (points.shape[0],), torch.inf, device=points.device, dtype=torch.float32
    )
...
def zip_vectors_kernel(f: Callable, n1: int, n2: int, out_n: int):
    @ti.kernel
    def k(
        vec1: ti.types.ndarray(dtype=ti.types.vector(n1, ti.f32), ndim=1),
...
def batch_distances(distance_func, obj1: torch.Tensor, obj2: torch.Tensor):
    ...
    distances = torch.full(
        (obj1.shape[0],), torch.inf, device=obj1.device, dtype=torch.float32
    )
